#ifndef LAHUTA_SOURCES_NMR_HPP
#define LAHUTA_SOURCES_NMR_HPP
  ...
  std::shared_ptr<RDGeom::POINT3D_VECT> acquire_buffer() const {
    ...
    if (!coord_pool_.empty()) {
      ...
      return std::shared_ptr<RDGeom::POINT3D_VECT>(
        base.get(),
        [weak_self = std::weak_ptr<const NMRSession>(shared_from_this()), base = std::move(base)](RDGeom::POINT3D_VECT*) mutable {
