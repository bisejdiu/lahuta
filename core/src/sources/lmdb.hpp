...
#include "sources/lmdb_coordinate_view.hpp"
...
public:
  ...
  LMDBSession(const LMDBRef& ref, std::string id, pipeline::DataFieldSet /*unused*/, std::size_t max_inflight = 1)
      : LMDBSession(ref, std::move(id), max_inflight) {}

  const std::string& key() const noexcept { return key_; }

  pipeline::ModelPayloadSlices model_payload(pipeline::DataFieldSet requested) const override {
    pipeline::ModelPayloadSlices slices;
    const bool wants_metadata       = requested.contains(pipeline::DataField::Metadata);
    const bool wants_plddt_copy     = requested.contains(pipeline::DataField::Plddt);
    const bool wants_plddt_view     = requested.contains(pipeline::DataField::PlddtView);
    const bool wants_dssp_copy      = requested.contains(pipeline::DataField::Dssp);
    const bool wants_dssp_view      = requested.contains(pipeline::DataField::DsspView);
    const bool wants_sequence_copy  = requested.contains(pipeline::DataField::Sequence);
    const bool wants_sequence_view  = requested.contains(pipeline::DataField::SequenceView);
    const bool wants_frame          = requested.contains(pipeline::DataField::Positions);
    const bool wants_positions_view = requested.contains(pipeline::DataField::PositionsView);
...
private:
  ...
  std::shared_ptr<CoordinateHandle> build_coordinate_handle(const LoadedPayload& payload, ReaderLease& lease) const {
    ...
    if (coords_slice.length == 0 && count == 0) {
      return std::make_shared<CoordinateHandle>(ReaderLease(lease), span<const RDGeom::Point3Df>{}, payload.payload);
    }
  ...
  analysis::system::ModelRecord deserialize_record(std::string_view raw) const {
    return serialization::Serializer<fmt::binary, analysis::system::ModelRecord>::deserialize(raw.data(), raw.size());
  }

  analysis::system::ModelRecord load_full_record() const {
    return with_record_value([this](std::string_view raw) {
  ...
  std::shared_ptr<const analysis::system::ModelRecord> ensure_record_loaded() const {
    std::call_once(record_once_, [this]() {
      record_ = std::make_shared<analysis::system::ModelRecord>(load_full_record());
    });
    return record_;
  }
...
class LMDBRealizer {
public:
  void set_requirements(pipeline::DataFieldSet req) { requirements_ = req; }
...
private:
  ...
  bool needs_session() const { return requirements_.contains_any(pipeline::SessionBoundFields); }
