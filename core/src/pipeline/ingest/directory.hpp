#ifndef LAHUTA_PIPELINE_INGEST_DIRECTORY_HPP
#define LAHUTA_PIPELINE_INGEST_DIRECTORY_HPP
...
public:
  ...
  explicit Directory(std::string_view directory_path, std::string_view extension = "", bool recursive = true,
                     std::size_t batch_size = 1024)
      : Directory(directory_path,
                  extension.empty() ? std::vector<std::string>{}
                                    : std::vector<std::string>{std::string(extension)},
                  recursive, batch_size) {}

  Directory(std::string_view directory_path, std::initializer_list<std::string_view> extensions,
            bool recursive = true, std::size_t batch_size = 1024)
      : Directory(directory_path, std::vector<std::string>(extensions.begin(), extensions.end()), recursive,
                  batch_size) {}

  Directory(std::string_view directory_path, std::vector<std::string> extensions, bool recursive = true,
            std::size_t batch_size = 1024)
      : directory_path_(directory_path), extensions_(normalize_extensions(std::move(extensions))),
        recursive_(recursive), batch_size_(batch_size) {

    if (!fs::exists(directory_path_) || !fs::is_directory(directory_path_)) {
      throw fs::filesystem_error("Directory does not exist",
                                 std::make_error_code(std::errc::no_such_file_or_directory));
    }

    if (recursive_) {
      recursive_it_.emplace(directory_path_, fs::directory_options::skip_permission_denied);
    } else {
      dir_it_.emplace(directory_path_, fs::directory_options::skip_permission_denied);
    }

    const auto filters = describe_filters();
    Logger::get_logger()->info("Directory: scanning directory '{}'{}; batch_size={} (recursive={})",
                               directory_path_,
...
private:
  bool load_next_batch() {
    ...
    try {
      if (recursive_) {
        while (recursive_it_.has_value() && *recursive_it_ != fs::recursive_directory_iterator{} &&
               loaded < batch_size_) {
...
#endif // LAHUTA_PIPELINE_INGEST_DIRECTORY_HPP
