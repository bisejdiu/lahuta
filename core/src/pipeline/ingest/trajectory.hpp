#ifndef LAHUTA_PIPELINE_INGEST_TRAJECTORY_HPP
#define LAHUTA_PIPELINE_INGEST_TRAJECTORY_HPP
...
public:
  ...
  TrajectorySession(std::string structure, std::vector<std::string> xtcs, std::string id,
  ...
  public:
    Frame(std::shared_ptr<const TrajectorySession> session, std::size_t global_idx,
          std::shared_ptr<const md::XtcReader::FramePacket> packet)
...
private:
  ...
  static StructureFormat detect_format(const std::string &path) {
    ...
    return ends_with(lower, ".gro") || ends_with(lower, ".gro.gz") ? StructureFormat::Gro
  ...
  std::shared_ptr<RDGeom::POINT3D_VECT> acquire_buffer() const {
    ...
    if (!coord_pool_.empty()) {
      ...
      return std::shared_ptr<RDGeom::POINT3D_VECT>(
          raw,
          [weak_self = std::weak_ptr<const TrajectorySession>(shared_from_this()),
...
public:
  ...
  std::optional<PipelineItem> next(const IngestDescriptor &desc) {
    ...
    while (true) {
      ...
      std::shared_ptr<const md::XtcReader::FramePacket>
          packet_ptr = std::make_shared<md::XtcReader::FramePacket>(std::move(packet));
...
#endif // LAHUTA_PIPELINE_INGEST_TRAJECTORY_HPP
