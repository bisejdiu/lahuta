...
template<typename Fn> auto stage(Fn&& fn)                  {return detail::make_stage(thread_unsafe, std::forward<Fn>(fn));}
template<typename Fn> auto stage(thread_safe_t,   Fn&& fn) {return detail::make_stage(thread_safe,   std::forward<Fn>(fn));}
template<typename Fn> auto stage(thread_unsafe_t, Fn&& fn) {return detail::make_stage(thread_unsafe, std::forward<Fn>(fn));}
...
#endif // LAHUTA_PIPELINE_DSL_STAGES_HPP
