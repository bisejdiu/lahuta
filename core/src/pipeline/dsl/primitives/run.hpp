...
#ifndef LAHUTA_PIPELINE_DSL_PRIMITIVES_RUN_HPP
#define LAHUTA_PIPELINE_DSL_PRIMITIVES_RUN_HPP
...
struct stages_tuple<pipe_t<L, R>> {
  using type = decltype(std::tuple_cat(stages_tuple<L>::get(std::declval<L &>()),
                                       stages_tuple<R>::get(std::declval<R &>())));
  static type get(pipe_t<L, R> &p) {
    return std::tuple_cat(stages_tuple<L>::get(p.lhs), stages_tuple<R>::get(p.rhs));
  }
};

// unwrap std::reference_wrapper if we are passed a collect(emitter)
template <typename T>
auto &unwrap_sink(T &s) {
  return s;
}
template <typename U>
auto &unwrap_sink(std::reference_wrapper<U> w) {
  return w.get();
}
...
#endif // LAHUTA_PIPELINE_DSL_PRIMITIVES_RUN_HPP
