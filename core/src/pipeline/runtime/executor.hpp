...
#ifndef LAHUTA_PIPELINE_RUNTIME_EXECUTOR_HPP
#define LAHUTA_PIPELINE_RUNTIME_EXECUTOR_HPP
...
#include "pipeline/runtime/engine.hpp"
...
public:
  ...
  using EngineT      = ComputeEngine<PipelineContext, Mut::ReadWrite>;
  ...
  struct ThreadLocalState {
    ...
    ~ThreadLocalState() { StageExecutor::cleanup_hook_(*this); }
  };

  using TlsCleanupHook = void (*)(ThreadLocalState &);

  StageExecutor(const CompiledStage &stage, ChannelMultiplexer &mux, std::size_t run_token, Metrics &metrics,
                DataFieldSet requirements, std::shared_ptr<IRunObserver> observer = {})
  ...
  static void reset_tls_cleanup_hook() { cleanup_hook_ = &StageExecutor::default_tls_cleanup; }

  static void set_plan_build_hook(PlanBuildHook hook) {
    plan_build_hook_.store(hook, std::memory_order_relaxed);
  }

  static void reset_plan_build_hook() { plan_build_hook_.store(nullptr, std::memory_order_relaxed); }
  ...
  void run(Source &src, std::size_t threads) {
    ...
    struct InflightGuard {
      Metrics &metrics;
      bool active{false};
      explicit InflightGuard(Metrics &m) : metrics(m), active(true) { metrics.on_item_inflight_enter(); }
      void release() {
        if (!active) return;
    ...
    Stage<PipelineItem, void> process_stage(
        [this, observer](PipelineItem item, IEmitter<void> &) {
          ...
          logger->debug("StageExecutor[run_token={}]: processing item session='{}' conformer={} path='{}'",
                        run_token_,
                        session_label,
                        item.conformer_id,
                        item.item_path);
          ...
          const bool prepared = prepare_item_state(item,
                                                   ctx,
                                                   tls_state.data,
                                                   permit,
                                                   shared_coords,
          ...
          if (!prepared) {
            logger->warn("StageExecutor[run_token={}]: skipping item session='{}' conformer={} due to "
                         "coordinate/setup failure",
                         run_token_,
                         session_label,
                         item.conformer_id);
          ...
          for (std::size_t index = 0; index < stage_.labels.size(); ++index) {
            ...
            if (!ok) {
              auto &err = res.error();
              logger->warn("StageExecutor[run_token={}]: stage '{}' failed for session='{}' conformer={} "
                           "path='{}': {}. "
...
private:
  ...
  static void clear_tls_state(ThreadLocalState &state) {
    ...
    state.engine.reset();
    state.data     = PipelineContext{};
    state.run_seen = 0;
    state.stage_cache.reset();
  }

  static void default_tls_cleanup(ThreadLocalState &state) { clear_tls_state(state); }

  void publish_required_handles(TaskContext &ctx, PipelineContext &data) const {
    ...
    //
    auto payload = std::make_shared<ModelPayloadSlices>(std::move(slices));
    ctx.set_object<ModelPayloadSlices>(CTX_MODEL_PAYLOAD_KEY, payload);
  }
  ...
  //
  bool prepare_item_state(const PipelineItem &item, TaskContext &t_ctx, PipelineContext &p_ctx,
                          std::unique_ptr<StreamSession::Permit> &permit,
                          std::shared_ptr<RDGeom::POINT3D_VECT> &shared_coords,
                          std::shared_ptr<RDKit::Conformer> &conformer, ThreadHandle &metrics_handle) {
    ...
    if (conformer) {
      t_ctx.set_object<RDKit::Conformer>(CTX_CONFORMER_KEY, conformer);
    }
    return true;
  }
  ...
  DataFieldSet requirements_ = DataFieldSet::none();
};

} // namespace lahuta::pipeline

#endif // LAHUTA_PIPELINE_RUNTIME_EXECUTOR_HPP
