...
void addHs(RWMol &mol, bool explicitOnly = false,
                                 bool addCoords = false,
                                 const UINT_VECT *onlyOnAtoms = nullptr,
                                 bool addResidueInfo = false);
...
//! \overload
/// modifies the molecule in place
void removeHs(RWMol &mol, const RemoveHsParameters &ps,
                                    bool sanitize = true);
//! \overload
...
void removeAllHs(RWMol &mol, bool sanitize = true);
//! \overload
/// The caller owns the pointer this returns
ROMol *removeAllHs(const ROMol &mol,
                                         bool sanitize = true);
...
void parseAdjustQueryParametersFromJSON(
...
*/
bool KekulizeIfPossible(RWMol &mol,
...
*/
std::list<int> getShortestPath(const ROMol &mol, int aid1,
                                                     int aid2);
...
//! \overload
void cleanupAtropisomers(RWMol &);
...
*/
void findPotentialStereoBonds(ROMol &mol,
                                                    bool cleanIt = false);
...
/*!
  ...
  \param replaceExistingTags  if this flag is true, any existing atomic chiral
                              tags will be replaced
*/
void assignChiralTypesFromMolParity(
    ROMol &mol, bool replaceExistingTags = true);

//! @}

//! returns the number of atoms which have a particular property set
unsigned getNumAtomsWithDistinctProperty(
 ...
 *
 */
void collapseAttachmentPoints(RWMol &mol,
 ...
 *
 */
unsigned int addExplicitAttachmentPoint(
 ...
 *
 */
bool isAttachmentPoint(const Atom *atom,
                                             bool markedOnly = true);
