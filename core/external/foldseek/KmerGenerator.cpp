...
//    calcDivideStrategy();
    ...
    this->stepMultiplicator = nullptr;
    this->highestScorePerArray = nullptr;
    this->possibleRest = nullptr;
    this->kmerIndex = nullptr;
    this->divideStep = nullptr;
    this->matrixLookup = nullptr;
    this->outputScoreArray = nullptr;
    this->outputIndexArray = nullptr;
}
...
KmerGenerator::~KmerGenerator() {
    if (stepMultiplicator) delete[] stepMultiplicator;
    if (highestScorePerArray) delete[] highestScorePerArray;
    if (possibleRest) delete[] possibleRest;
    if (kmerIndex) delete[] kmerIndex;
    if (divideStep) delete[] divideStep;
    if (matrixLookup) delete[] matrixLookup;

    if (outputScoreArray) {
        for(size_t i = 0; i < 2; i++) {
            if (outputScoreArray[i]) free(outputScoreArray[i]);
        }
    ...
    if (outputIndexArray) {
        for(size_t i = 0; i < 2; i++) {
            if (outputIndexArray[i]) free(outputIndexArray[i]);
        }
