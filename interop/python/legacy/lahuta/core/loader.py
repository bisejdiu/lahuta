...
from lahuta.core.topology.loaders import LahutaSystemLoader, TopologyLoader, Loader
...
class GemmiLoader:
    def __init__(self, file_path: str) -> None:  # -> LahutaSystem:
    ...
    @staticmethod
    def from_ir(ir: IR) -> LahutaSystem:
...
if __name__ == "__main__":
    ...
    # 3. Convert the IR instance to LahutaSystem
    lahuta_from_ir = loader.from_ir(ir_instance)
    print(f"3.: {lahuta_from_ir.names.shape}", lahuta_from_ir)
    # 4. Convert the LahutaSystem instance to AtomGroupType
    ...
    # ir_instance = loader.to_ir()
    # 6. Convert IR instance to LahutaSystem
    lahuta_from_ir = loader.from_ir(ir_instance)
