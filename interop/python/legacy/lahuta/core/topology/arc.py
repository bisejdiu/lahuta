...
(either LahutaSystemLoader or TopologyLoader), it uses the appropriate method from Atoms, Residues, and
...
Example:
    ``` py
    >>> from arc import ARC, LahutaSystemLoader
    >>> loader = LahutaSystemLoader("path_to_structure")
...
from lahuta.lib._lahuta import LahutaSystem as cLuni
from lahuta._types.mdanalysis import AtomGroupType, UniverseType

if TYPE_CHECKING:
    from .loaders import LahutaSystemLoader, TopologyLoader, GemmiLoader
...
class ARC:
    ...
    Depending on the type of the input object (either LahutaSystemLoader or TopologyLoader), it uses the
    ...
    def __init__(
        self,
        obj_type: Union["LahutaSystemLoader", "TopologyLoader", "GemmiLoader"],
