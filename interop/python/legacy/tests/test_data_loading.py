...
def run_per_file_tests(key: str, luni: Luni) -> None:
    ...
    assert dict(zip(unique, counts, strict=True)) == EXPECTED_DATA[key]["element_count"]

    # resname counts
    unique, counts = np.unique(luni.arc.residues.resnames, return_counts=True)
    assert dict(zip(unique, counts, strict=True)) == EXPECTED_DATA[key]["resname_count"]
    ...
    most_common_names = dict(zip(top10[:, 0], top10[:, 1], strict=True))
...
def test_read_pdb(pdb_name: str, pdb_class: type[BaseFile], pdb_flag: bool) -> None:
    luni = read_luni(pdb_class(pdb=pdb_flag).file_loc)
...
def test_univ_init(pdb_name: str, pdb_class: type[BaseFile], pdb_flag: bool) -> None:
    if not pdb_flag:
        return
    luni = read_luni_from_mda(pdb_class(pdb=pdb_flag).file_loc)
    run_per_file_tests(pdb_name, luni)
