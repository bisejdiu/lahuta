...
def _compute_arpeggio_contacts(structure_path: Path) -> dict[str, Any]:
    pipeline = Pipeline(FileSource(str(structure_path)))
    pipeline.add_task(
        name="ag",
        task=ContactTask(provider=lxx.ContactProvider.Arpeggio),
    )
    output = pipeline.run(threads=1)
    records = output.to_dict("ag")
...
def _validate_basic_metadata(computed: dict[str, Any], expected: dict[str, Any], structure_name: str) -> None:
    ...
    assert str(computed.get("provider", "")).lower() == "arpeggio", (
        f"{structure_name}: Expected provider 'arpeggio', got '{computed.get('provider')}'"
    )
...
def test_arpeggio_contact_count_sanity_check(data_dir: Path) -> None:
    structure_path = data_dir / "1kx2_small.cif"

    if not structure_path.exists():
        pytest.skip(f"Test structure not found: {structure_path}")

    computed_data = _compute_arpeggio_contacts(structure_path)

    total_contacts = len(computed_data.get("contacts", []))
    assert total_contacts > 400, (
        f"Expected more than 400 contacts for 1kx2_small.cif with Arpeggio, got {total_contacts}"
    )

    counts = _count_by_interaction_type(computed_data)
    assert len(counts) >= 10, f"Expected at least 10 interaction types from Arpeggio, got {len(counts)}"

    expected_types = {"HydrogenBond", "Hydrophobic", "VanDerWaals"}
...
@pytest.mark.parametrize(
    ...
    [
        ("1kx2_small.cif", "1kx2_arpeggio.json"),
        ("1tqn.cif", "1tqn_arpeggio.json"),
        ("models/AF-Q57552-F1-model_v4.cif.gz", "Q57552_arpeggio.json"),
    ],
)
def test_arpeggio_contacts_regression(data_dir: Path, structure_file: str, expected_file: str) -> None:
    """
    Test that Arpeggio contact computation produces consistent results.
