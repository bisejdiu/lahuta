...
#
# Contact:
#     print("besian".__add__("sejdiu").__add__("@gmail.com"))
#
from __future__ import annotations

from lahuta.lib.lahuta import pipeline as _p
...
def test_stall_accumulates_under_blocking_policy() -> None:
    ...
    src = _p.sources.FileSource(items)
    sm = _p.StageManager(src)
    sm.set_reporting_level(_p.ReportingLevel.DEBUG)

    big = "Z" * (512 * 1024)  # = 512 KiB

    def emit(_: _p.PipelineContext) -> str:
        return big

    sm.add_python("emit", [], emit, "emit", serialize=True, store=True)

    ms = _p.SlowSink(0.005)
    cfg = _p.BackpressureConfig()
    cfg.max_queue_msgs = 1
    cfg.max_queue_bytes = len(big)
    cfg.max_batch_bytes = len(big)
    cfg.on_full = _p.OnFull.Block
