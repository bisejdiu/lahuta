...
try:
    ...
    print((
        f"\n{'=x'*29}=\n"
        f"ERROR: Lahuta C++ bindings could not be imported!\n"
        f"Import error: {e}\n\n"
        f"This package requires compiled C++ extensions to function.\n"
        f"Please ensure that the C++ core has been built successfully.\n"
        f"For installation instructions, see INSTALL.md\n"
        f"{'=x'*29}=\n"
    ), file=sys.stderr)
