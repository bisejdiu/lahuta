...
inline std::string make_report_path(const std::string& prefix, std::size_t run_token, const std::string& timestamp) {
  return prefix + "_run_report_" + std::to_string(run_token) + '_' + timestamp + ".json";
}

inline bool write_run_report_json(const std::string& path, const pipeline::dynamic::StageManager::RunReport& report) {
