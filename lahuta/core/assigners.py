...
class VectorizedProteinTypeAssigner(ProteinTypeAssignerBase):
    ...
    def compute(self, atypes_array, atypes):
        ...
        resname_str = resname[self.protein_atomgroup.atoms.resindices].astype(str)
        atom_name_str = atom_name[self.protein_atomgroup.atoms.indices].astype(str)
        ...
        prot_atom_types_array = [list(atom_ids_label_set) for atom_ids_label_set in PROT_ATOM_TYPES.values()]

        mask = np.array([np.isin(atom_id_labels, prot_atom_types) for prot_atom_types in prot_atom_types_array])
