...
//! \overload
ATOM_EQUALS_QUERY *makeAtomTypeQuery(int num,
                                                           int aromatic);
...
//! \overload
ATOM_EQUALS_QUERY *makeAtomNegativeFormalChargeQuery(
    int what);
...
//! \overload
ATOM_EQUALS_QUERY *makeAtomNumRadicalElectronsQuery(
    int what);
...
const std::vector<std::string> complexQueries = {"A", "AH", "Q", "QH",
                                                 "X", "XH", "M", "MH"};
void convertComplexNameToQuery(Atom *query,
...
//! \overload
ATOM_EQUALS_QUERY *makeAtomNumHeteroatomNbrsQuery(
    int what);
...
//! \overload
ATOM_EQUALS_QUERY *makeAtomNonHydrogenDegreeQuery(
    int what);
...
BOND_EQUALS_QUERY *makeBondOrderEqualsQuery(
    Bond::BondType what);
//! returns a Query for unspecified SMARTS bonds
BOND_EQUALS_QUERY *makeSingleOrAromaticBondQuery();
//! returns a Query for double|aromatic bonds
BOND_EQUALS_QUERY *makeDoubleOrAromaticBondQuery();
//! returns a Query for single|double bonds
BOND_EQUALS_QUERY *makeSingleOrDoubleBondQuery();
//! returns a Query for tautomeric bonds
BOND_EQUALS_QUERY *
makeSingleOrDoubleOrAromaticBondQuery();

//! returns a Query for matching bond directions
BOND_EQUALS_QUERY *makeBondDirEqualsQuery(
    Bond::BondDir what);
