...
bool MemoryMapped::open(const std::string& filename, size_t mappedBytes, CacheHint hint)
{
  ...
  if (!_mappedFile)
    ...
    // We need it all, and we need it now!     - Besian, March 2025
    if (_mappedView != NULL && _mappedView != _emptyView) {
        int ret = madvise(_mappedView, _mappedBytes, MADV_WILLNEED);
        if (ret != 0) {
            //Debug(Debug::ERROR) << "madvise failed: " << strerror(errno) << "\n";
            return false;
        }
    }
...
unsigned char MemoryMapped::at(size_t offset) const
{
    ...
    if (offset >= _filesize) {
            ...
            throw std::out_of_range("MemoryMapped::at");
    }
