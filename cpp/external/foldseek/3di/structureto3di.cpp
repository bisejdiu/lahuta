...
Alphabet3Di::Vec3 Alphabet3Di::StructureTo3DiBase::add(Vec3 a, Vec3 b){
    a.x = a.x + b.x;
    a.y = a.y + b.y;
    a.z = a.z + b.z;
    return a;
...
Alphabet3Di::Vec3 Alphabet3Di::StructureTo3DiBase::sub(Vec3 a, Vec3 b){
    a.x = a.x - b.x;
    a.y = a.y - b.y;
    a.z = a.z - b.z;
    return a;
...
Alphabet3Di::Vec3 Alphabet3Di::StructureTo3DiBase::norm(Vec3 a){
...
Alphabet3Di::Vec3 Alphabet3Di::StructureTo3DiBase::cross(Vec3 a, Vec3 b){
    return {
        a.y * b.z - a.z * b.y,
        a.z * b.x - a.x * b.z,
        a.x * b.y - a.y * b.x};
...
Alphabet3Di::Vec3 Alphabet3Di::StructureTo3DiBase::scale(Vec3 a, double f){
    a.x *= f;
    a.y *= f;
    a.z *= f;
    return a;
...
double Alphabet3Di::StructureTo3DiBase::dot(Vec3 a, Vec3 b){
    return a.x * b.x + a.y * b.y + a.z * b.z;
}

Alphabet3Di::Vec3 Alphabet3Di::StructureTo3DiBase::approxCBetaPosition(Vec3 ca_atom, Vec3 n_atom, Vec3 c_atom){
...
double Alphabet3Di::StructureTo3DiBase::degreeToRadians(double degree){
    return (degree / 180) * Alphabet3Di::PI;
}

Alphabet3Di::Vec3 Alphabet3Di::StructureTo3DiBase::calcVirtualCenter(Vec3 ca, Vec3 cb, Vec3 n,
...
double Alphabet3Di::StructureTo3DiBase::calcDistanceBetween(Vec3 & a, Vec3 & b){
...
void Alphabet3Di::StructureTo3DiBase::replaceCBWithVirtualCenter(Vec3 * ca, Vec3 * n,
...
void Alphabet3Di::StructureTo3DiBase::createResidueMask(std::vector<bool> & validMask,
...
void Alphabet3Di::StructureTo3DiBase::findResiduePartners(std::vector<int> & partnerIdx, Vec3 * cb,
...
Alphabet3Di::StructureTo3Di::Feature Alphabet3Di::StructureTo3Di::calcFeatures(Vec3 * ca, int i, int j){
...
void Alphabet3Di::StructureTo3Di::calcConformationDescriptors(std::vector<Feature> & features, std::vector<int> & partnerIdx,
...
void Alphabet3Di::StructureTo3Di::encodeFeatures(std::vector<Embedding> & embeddings, std::vector<Feature> & features,
...
void Alphabet3Di::StructureTo3Di::discretizeEmbeddings(std::vector<char> & states, std::vector<Embedding> & embeddings,
                                        std::vector<bool> & mask, const size_t len){
    double minDistance;
    char closestState;
...
char * Alphabet3Di::StructureTo3Di::structure2states(Vec3 * ca, Vec3 * n,
                                        Vec3 * c, Vec3 * cb,
                                        size_t len){
    features.clear();
    states.clear();
