...
public:
    IndexTable(int alphabetSize, int kmerSize, bool externalData)
        ...
        if (externalData == false) {
            ...
            // (bisejdiu): faster memory allocation
            offsets = static_cast<size_t*>(calloc(tableSize + 1, sizeof(size_t)));
            Util::checkAllocation(offsets, "Can not allocate entries memory in IndexTable");
        }
    }
    ...
    inline IndexEntryLocal *getDBSeqList(size_t kmer, size_t *matchedListSize) {
        // std::cout << "1. ? " << *offsets << " " << kmer << " " << tableSize << std::endl;
    ...
    void sortDBSeqLists(ctpl::thread_pool& pool) {
        size_t num_threads = pool.size();
        std::vector<std::future<void>> futures;

        size_t chunk_size = (tableSize + num_threads - 1) / num_threads;

        for (size_t t = 0; t < num_threads; ++t) {
            size_t start = t * chunk_size;
            size_t end = std::min(start + chunk_size, tableSize);

            if (start >= end) continue;

            futures.push_back(pool.push([this, start, end](int id) {
    ...
    void initMemory(size_t dbSize, ctpl::thread_pool& pool) {
        size_t num_threads = pool.size();
        ...
        for (size_t t = 0; t < num_threads; ++t) {
            ...
            futures.push_back(pool.push([this, start, end](int id) -> size_t {
                size_t local_sum = 0;
                for (size_t i = start; i < end; ++i) {
                    local_sum += getOffset(i);
                }
                return local_sum;
            }));
        }
        ...
        // allocate memory for the sequence id lists
        entries = new(std::nothrow) IndexEntryLocal[tableEntriesNum];
        Util::checkAllocation(entries, "Cannot allocate entries memory in IndexTable::initMemory");
    }
    ...
    void init(ctpl::thread_pool& pool) {
        size_t num_threads = pool.size();
        std::vector<std::future<size_t>> futures;
        std::vector<size_t> chunk_sums(num_threads);
        ...
        for (size_t t = 0; t < num_threads; ++t) {
            ...
            futures.push_back(pool.push([this, start, end, t, &chunk_sums](int id) {
                size_t local_sum = 0;
                for (size_t i = start; i < end; ++i) {
                    const size_t currentOffset = offsets[i];
                    offsets[i] = local_sum;
                    local_sum += currentOffset;
                }
        ...
        std::vector<size_t> chunk_offsets(num_threads);
        size_t running_sum = 0;
        for (size_t t = 0; t < num_threads; ++t) {
            chunk_offsets[t] = running_sum;
            running_sum += chunk_sums[t];
        }
        ...
        for (size_t t = 0; t < num_threads; ++t) {
            ...
            futures.push_back(pool.push([this, start, end, t, chunk_offsets](int id) {
                size_t total_offset = chunk_offsets[t];
                for (size_t i = start; i < end; ++i) {
