...
SubstitutionMatrix::SubstitutionMatrix(const CompileTimeMatrix &matrix, float bitFactor, float scoreBias) : bitFactor(bitFactor) {
    using namespace lahuta;

    matrixName = std::string_view(matrix.name, matrix.name_size);
    matrixData = std::string_view(matrix.get_data_as_char(), matrix.size);
