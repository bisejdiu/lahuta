...
QueryMatcher::QueryMatcher(IndexTable *indexTable, SequenceLookup *sequenceLookup,
        ...
        : idx(indexTable->getAlphabetSize(), kmerSize), isNucleotide(isNucleotide), hook(NULL), activeCounter(0)
{
...
//    this->diagonalMatcher = new CacheFriendlyOperations(dbSize, maxDbMatches / 128 );
    ...
    // cachedOperation2 = nullptr;
    // cachedOperation4 = nullptr;
    // cachedOperation8 = nullptr;
    // cachedOperation16 = nullptr;
    // cachedOperation32 = nullptr;
    // cachedOperation64 = nullptr;
    // cachedOperation128 = nullptr;
    // cachedOperation256 = nullptr;
    // cachedOperation512 = nullptr;
    // cachedOperation1024 = nullptr;
    // cachedOperation2048 = nullptr;

}

// QueryMatcher::~QueryMatcher() {
// if (resList) free(resList);
// if (scoreSizes) delete[] scoreSizes;
// if (databaseHits) delete[] databaseHits;
// if (indexPointer) delete[] indexPointer;
// if (foundDiagonals) free(foundDiagonals);
// if (compositionBias) delete[] compositionBias;
// if (ungappedAlignment) delete ungappedAlignment;
// if (stats) delete stats;
// if (kmerGenerator) delete kmerGenerator;
//
// }
...
std::pair<hit_t*, size_t> QueryMatcher::matchQuery(Sequence *querySeq, unsigned int identityId, bool isNucleotide) {
    ...
    if (diagonalScoring) {
        ...
        // std::cout << "Y. Diag: " << foundDiagonals[0].diagonal << std::endl;
        // std::cout << "Y. Diag: " << foundDiagonals[1].diagonal << std::endl;
        // std::cout << "Y. Diag: " << foundDiagonals[2].diagonal << std::endl;
        // std::cout << "Y. Diag: " << foundDiagonalsSize << std::endl;
...
size_t QueryMatcher::match(Sequence *seq, float *compositionBias) {
    ...
    const auto &[f, s] = seq->getSpacedPattern(true, 6);
...
void QueryMatcher::initDiagonalMatcher(size_t dbsize, unsigned int maxDbMatches) {
        activeCounter = 0;  // Reset to known state
